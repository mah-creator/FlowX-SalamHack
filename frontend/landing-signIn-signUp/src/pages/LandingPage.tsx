...
export default function LandingPage() {
  ...
  const [showLoader, setShowLoader] = useState(!hasShownLandingLoader && !prefersReducedMotion);
  const [isLoaded, setIsLoaded] = useState(hasShownLandingLoader || prefersReducedMotion);
  ...
  return (
    <>
      {showLoader ? <Loader progress={progress} onComplete={handleLoaderComplete} /> : null}

      <div className="relative flex min-h-screen flex-col bg-white font-sans text-brand-blue selection:bg-brand-teal/30">
        <div className="relative flex min-h-screen flex-col overflow-hidden lg:h-screen" ref={heroViewportRef}>
          <video
            autoPlay
            loop
            muted
          ...
          <div className="relative z-10 flex min-h-screen flex-col lg:h-screen" id="top">
