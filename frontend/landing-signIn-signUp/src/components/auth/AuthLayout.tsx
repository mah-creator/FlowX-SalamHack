...
function renderWords(text: string, className: string) {
  return text.split(' ').map((word, i) => (
    <span key={`${word}-${i}`} className={`mr-3 inline-block ${className}`} data-tagline-word>
      {word}
    </span>
  ));
}
...
export function AuthLayout({ children }: AuthLayoutProps) {
  ...
  return (
    <div ref={layoutRef} className="flex min-h-screen font-sans selection:bg-brand-teal/30">
      <div
