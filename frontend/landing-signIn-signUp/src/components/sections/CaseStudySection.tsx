...
function PersonCard({ person, side, cardRef }: PersonCardProps) {
  return (
    ...
    >
      ...
      <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.2em] text-brand-teal" data-location>
        {person.location}
      </p>
    ...
    return (
      <div className="relative mx-auto w-full max-w-sm py-4" data-corridor>
        <svg viewBox="0 0 80 220" className="mx-auto h-auto w-20" fill="none">
          <line x1="40" y1="24" x2="40" y2="196" stroke="rgba(255,255,255,0.1)" strokeWidth="2" strokeDasharray="8 6" />
          <line
            x1="40"
            y1="24"
            x2="40"
          ...
          />
          <circle cx="40" cy="24" r="6" fill="#0a0a0a" stroke="#0094ac" strokeWidth="1.5" data-node-left />
          <circle cx="40" cy="196" r="6" fill="#0a0a0a" stroke="#0094ac" strokeWidth="1.5" data-node-right />
          <circle cx="40" cy="24" r="4" fill="#0094ac" data-traveling-dot />

          <rect x="10" y="92" width="60" height="34" rx="17" fill="#0094ac" data-flowx-badge opacity="0" />
          <text
            x="40"
            y="113"
            textAnchor="middle"
  ...
  return (
    <div className="relative mx-auto w-full max-w-3xl py-8" data-corridor>
      <svg viewBox="0 0 600 60" className="h-auto w-full" fill="none">
        <line x1="40" y1="30" x2="560" y2="30" stroke="rgba(255,255,255,0.08)" strokeWidth="2" strokeDasharray="8 6" />
        <line
          x1="40"
          y1="30"
          x2="560"
  ...
  useGSAP(
    () => {
      ...
      const buildTimeline = (isMobile: boolean) => {
        const corridorRoot = isMobile ? corridorMobileRef.current : corridorDesktopRef.current;
        const corridorPath = corridorRoot?.querySelector<SVGLineElement>('[data-corridor-path]') ?? null;
        const travelingDot = corridorRoot?.querySelector<SVGCircleElement>('[data-traveling-dot]') ?? null;
        const nodeLeft = corridorRoot?.querySelector<SVGCircleElement>('[data-node-left]') ?? null;
        const nodeRight = corridorRoot?.querySelector<SVGCircleElement>('[data-node-right]') ?? null;
        const badge = corridorRoot?.querySelector<SVGRectElement>('[data-flowx-badge]') ?? null;
        const badgeText = corridorRoot?.querySelector<SVGTextElement>('[data-flowx-text]') ?? null;
        ...
        if (reduceMotion) {
          tl.from(overlineRef.current, { opacity: 0, duration: 0.3 })
            .from(headingWordsRef.current, { opacity: 0, duration: 0.3, stagger: 0.06 }, '-=0.1')
            .from([leftCardRef.current, rightCardRef.current], { opacity: 0, duration: 0.35, stagger: 0.12 }, '-=0.05')
            .from(corridorRoot, { opacity: 0, duration: 0.3 }, '-=0.1')
            .from(matchLineRef.current, { opacity: 0, duration: 0.25 }, '-=0.05')
        ...
        tl.to(nodeLeft, { attr: { r: 8 }, duration: 0.3, ease: 'power2.out' }, '-=0.8').to(nodeLeft, {
          attr: { r: 6 },
          duration: 0.2,
        });

        tl.to(nodeRight, { attr: { r: 8 }, duration: 0.3, ease: 'power2.out' }, '-=0.6').to(nodeRight, {
          attr: { r: 6 },
          duration: 0.2,
        });
        ...
        tl.to([badge, badgeText], { opacity: 1, duration: 0.3, ease: 'power2.out' }, '-=0.4');
    ...
    >
      ...
      <Container className="relative z-10">
        <div className="mx-auto max-w-5xl text-center">
          ...
          <p className="mb-14 text-3xl italic tracking-tight text-white/30 md:mb-16 md:text-4xl" data-heading-accent>
        ...
        <div className="mb-5 flex flex-col gap-8 md:mb-4 md:flex-row md:justify-between md:gap-10">
          ...
          <PersonCard person={rightPerson} side="right" cardRef={rightCardRef} />
        </div>
        ...
        >
          ...
          <span className="text-sm font-medium text-brand-teal">{caseStudy.matchLine}</span>
        </div>
        ...
        >
          <div className="mb-2 flex items-center justify-center gap-2 text-brand-teal">
            ...
            <p className="text-sm font-bold uppercase tracking-[0.2em]">{caseStudy.resultTitle}</p>
          </div>
          <p className="text-xs italic text-white/45">{caseStudy.resultSubtitle}</p>
        </div>
      </Container>
    </section>
  );
