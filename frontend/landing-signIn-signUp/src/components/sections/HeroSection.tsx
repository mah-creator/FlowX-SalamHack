...
function renderWords(text: string, attr: 'data-hero-word-1' | 'data-hero-word-2') {
  return text.split(' ').map((word, i) => (
    <span key={`${word}-${i}`} className="mr-2 inline-block overflow-hidden md:mr-3">
...
export function HeroSection() {
  return (
    <Container as="main" className="relative flex flex-grow flex-col justify-center pb-0 pt-12 md:pt-24" data-hero-section>
      <div className="relative z-10 grid grid-cols-1 items-center gap-12 lg:grid-cols-2 lg:gap-24" data-hero-content>
        <div>
        ...
        <div className="flex flex-col gap-6 md:gap-8 lg:ml-auto lg:max-w-md lg:items-start lg:text-left">
          <p data-hero-subtitle className="gsap-hidden max-w-sm text-sm font-light leading-relaxed text-brand-blue/90 md:text-base">
            FlowX matches local payment needs to make cross-border transfers faster, cheaper, and more reliable — even in restricted regions.
          </p>
