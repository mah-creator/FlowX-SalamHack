...
function FeatureVisual({ type }: FeatureVisualProps) {
  const placeholders: Record<FeatureIconType, string> = {
    matching: 'https://placehold.co/1200x520/eaf7fb/1a2e4c?text=Smart+Matching+Preview',
    escrow: 'https://placehold.co/1200x520/eaf7fb/1a2e4c?text=Escrow+Security+Preview',
    tracking: 'https://placehold.co/1200x520/eaf7fb/1a2e4c?text=Real-Time+Tracking+Preview',
    currency: 'https://placehold.co/1200x520/eaf7fb/1a2e4c?text=Multi-Currency+Preview',
    dispute: 'https://placehold.co/1200x520/eaf7fb/1a2e4c?text=Dispute+Resolution+Preview',
    scores: 'https://placehold.co/1200x520/eaf7fb/1a2e4c?text=Verification+Scores+Preview',
  };
...
export function FeaturesSection() {
  ...
  const switchFeature = (newIndex: number) => {
    ...
    switchTimelineRef.current = gsap.timeline({
      onComplete: () => {
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`text-sm font-semibold ${
                      i === activeIndex ? 'text-brand-blue' : 'text-brand-blue/70'
                    }`}
                  >
                    {feature.title}
                  </span>
                </div>
                <p className="mt-3 text-sm leading-relaxed text-brand-blue/65">{feature.desc}</p>
              </button>
            ))}
          </div>
        </div>
