...
export function FAQSection() {
  ...
  const renderWords = (text: string, className: string) =>
    text.split(' ').map((word, i) => (
      <span key={`${word}-${i}`} className="mr-3 inline-block overflow-hidden">
        <span ref={addHeadingRef} className={`inline-block ${className}`} style={{ willChange: 'transform, opacity' }}>
          {word}
        </span>
      </span>
    ));
  ...
  return (
    <section ref={sectionRef} id="faq" className="bg-[#FAFAFA] py-24 text-brand-blue md:py-32">
      <div className="mx-auto w-full max-w-[1250px] px-6 md:px-12">
        <div className="grid grid-cols-1 gap-14 lg:grid-cols-[35%_1fr] lg:gap-24">
          <div className="self-start lg:sticky lg:top-32">
            ...
            <div ref={counterRef} className="mb-8 flex items-baseline gap-3" data-counter>
            ...
            <div ref={accentLineRef} className="mb-8 h-[2px] w-12 bg-brand-teal" data-accent-line />

            <div ref={supportCtaRef} data-support-cta>
              <p className="mb-3 text-sm font-light text-brand-blue/50">Still have a question?</p>
              <a
