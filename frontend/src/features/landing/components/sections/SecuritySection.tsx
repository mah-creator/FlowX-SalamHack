...
function SecurityIcon({ type }: { type: SecurityPillar['icon'] }) {
  const paths: Record<SecurityPillar['icon'], string> = {
    identity: 'M16 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2m10-10a4 4 0 1 0-8 0 4 4 0 0 0 8 0m2 6 2 2 4-4',
    escrow: 'M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10',
    admin: 'M1.5 12s3.8-7 10.5-7 10.5 7 10.5 7-3.8 7-10.5 7S1.5 12 1.5 12m10.5-3a3 3 0 1 0 0 6 3 3 0 0 0 0-6',
    fraud: 'M13 16h-1v-4h-1m1-4h.01M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20',
  };

  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-5 w-5 text-brand-teal" data-icon-svg>
...
function ShieldCore() {
  return (
    <div className="relative mx-auto h-48 w-48 md:h-64 md:w-64 lg:h-80 lg:w-80" data-shield-core>
...
function PillarCard({ pillar, enableTilt }: PillarCardProps) {
  const handleMouseMove = (e: MouseEvent<HTMLDivElement>) => {
    if (!enableTilt || window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  ...
  return (
    ...
    >
      <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-brand-teal/10" data-pillar-icon>
        <SecurityIcon type={pillar.icon} />
      </div>

      <h4 className="mb-2 text-sm font-semibold tracking-tight text-white" data-pillar-title>
        {pillar.t}
      </h4>

      <p className="text-xs font-light leading-relaxed text-white/50" data-pillar-desc>
        {pillar.longDesc}
      </p>
...
export function SecuritySection() {
  ...
  const renderWords = (text: string, className: string) =>
    text.split(' ').map((word, i) => (
      <span key={`${word}-${i}`} className="mr-3 inline-block overflow-hidden">
        <span ref={addHeadingWordRef} className={`inline-block ${className}`} style={{ willChange: 'transform, opacity' }}>
          {word}
        </span>
      </span>
    ));
  ...
  const canHoverTilt = typeof window !== 'undefined' && window.matchMedia('(hover: hover) and (pointer: fine)').matches;

  const pillarsByPosition = securityPillars.reduce<Record<SecurityPillar['position'], SecurityPillar | null>>(
    (acc, pillar) => {
  ...
  useGSAP(
    () => {
      ...
      const buildTimeline = (isDesktop: boolean) => {
        const shieldRoot = isDesktop ? desktopShieldRef.current : mobileShieldRef.current;
        const pillarRoots = (isDesktop ? desktopPillarRefs.current : mobilePillarRefs.current).filter(Boolean);

        if (!shieldRoot || pillarRoots.length === 0) return;

        const rings = shieldRoot.querySelectorAll<SVGCircleElement>('[data-ring-1], [data-ring-2], [data-ring-3]');
        const ringTwo = shieldRoot.querySelector<SVGCircleElement>('[data-ring-2]');
        const ringThree = shieldRoot.querySelector<SVGCircleElement>('[data-ring-3]');
        const shieldOutline = shieldRoot.querySelector<SVGPathElement>('[data-shield-outline]');
        const shieldCheck = shieldRoot.querySelector<SVGPathElement>('[data-shield-check]');
        const shieldGlow = shieldRoot.querySelector<SVGCircleElement>('[data-shield-glow]');
        ...
        if (reduceMotion) {
          tl.from(overlineRef.current, { opacity: 0, duration: 0.25 })
            .from(headingWordsRef.current, { opacity: 0, duration: 0.25, stagger: 0.05 }, '-=0.08')
            .from(shieldRoot, { opacity: 0, duration: 0.3 }, '-=0.08')
            .from(pillarRoots, { opacity: 0, duration: 0.25, stagger: 0.08 }, '-=0.08')
        ...
        pillarRoots.forEach((card) => {
          ...
          tl.from(icon, { scale: 0, opacity: 0, duration: 0.3, ease: 'back.out(2)' }, '-=0.35')
    ...
    >
      <div className="pointer-events-none absolute inset-0">
        ...
        <div
          ...
          style={{
            background: 'radial-gradient(circle, rgba(0,148,172,0.08) 0%, transparent 70%)',
          }}
        />
      </div>
      ...
      <Container className="relative z-10">
        <div className="mx-auto max-w-4xl text-center">
          <span ref={overlineRef} className="mb-5 block text-xs font-bold uppercase tracking-[0.25em] text-brand-teal">
            Security
          </span>
          ...
          <p className="mt-2 text-3xl font-light tracking-tight text-white/30 md:text-4xl" data-heading-accent>
            {renderWords('& Verification', 'text-white/30')}
          </p>
        </div>

        <div className="relative mt-14 hidden h-[42rem] max-w-5xl mx-auto lg:block" data-diamond>
          <svg viewBox="0 0 100 100" className="pointer-events-none absolute inset-0 h-full w-full" fill="none" aria-hidden="true">
            <line x1="50" y1="50" x2="50" y2="18" stroke="rgba(0,148,172,0.18)" strokeWidth="0.5" strokeDasharray="2 2" />
            <line x1="50" y1="50" x2="82" y2="50" stroke="rgba(0,148,172,0.18)" strokeWidth="0.5" strokeDasharray="2 2" />
            <line x1="50" y1="50" x2="18" y2="50" stroke="rgba(0,148,172,0.18)" strokeWidth="0.5" strokeDasharray="2 2" />
            <line x1="50" y1="50" x2="50" y2="82" stroke="rgba(0,148,172,0.18)" strokeWidth="0.5" strokeDasharray="2 2" />

            <line x1="50" y1="50" x2="50" y2="18" stroke="rgba(0,148,172,0.42)" strokeWidth="0.7" data-conn-active strokeDasharray="160" strokeDashoffset="160" />
            <line x1="50" y1="50" x2="82" y2="50" stroke="rgba(0,148,172,0.42)" strokeWidth="0.7" data-conn-active strokeDasharray="160" strokeDashoffset="160" />
            <line x1="50" y1="50" x2="18" y2="50" stroke="rgba(0,148,172,0.42)" strokeWidth="0.7" data-conn-active strokeDasharray="160" strokeDashoffset="160" />
            <line x1="50" y1="50" x2="50" y2="82" stroke="rgba(0,148,172,0.42)" strokeWidth="0.7" data-conn-active strokeDasharray="160" strokeDashoffset="160" />
          ...
          <div ref={desktopShieldRef} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
            <ShieldCore />
          </div>
        ...
        <div className="mt-12 space-y-4 lg:hidden">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {securityPillars.map((pillar, i) => (
              <div key={`mobile-pillar-${pillar.t}`} ref={(el) => setMobilePillarRef(el, i)}>
        ...
        <div className="mt-12 flex flex-col items-center justify-center">
          <div ref={taglineLineRef} className="mb-5 h-px w-[110px] bg-brand-teal/70" />
          <p ref={taglineRef} className="text-center text-sm font-light italic text-white/35">
