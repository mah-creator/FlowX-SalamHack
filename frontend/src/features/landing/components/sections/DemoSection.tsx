...
import { demoStatuses, demoTransaction, type DemoStatus } from '../../data/constants';
  ...
  return (
    <div className="flex items-start gap-3 md:flex-1" data-step={index}>
      <div className="relative mt-0.5 shrink-0" data-step-dot>
        <div className="flex h-3 w-3 items-center justify-center rounded-full bg-white/20" data-step-core />
        <div className="absolute inset-0 rounded-full ring-4 ring-brand-teal/20 opacity-0" data-step-ring />
      </div>
      ...
      <div>
        ...
        <span className="mt-1 block text-[10px] font-mono tabular-nums text-white/0" data-step-time>
          {status.timestamp}
        </span>
      </div>
      ...
      {!isLast && (
        <>
          <div className="mx-2 mt-[7px] hidden h-px flex-1 bg-white/[0.06] md:block" data-connector />
          <div className="ml-[5px] mt-2 h-5 w-px bg-white/[0.06] md:hidden" data-connector-mobile />
        </>
      )}
...
export function DemoSection() {
  ...
  useGSAP(
    () => {
      ...
      const connectors = Array.from(section.querySelectorAll<HTMLElement>('[data-connector], [data-connector-mobile]'));
      ...
      const setCompleteState = () => {
        ...
        stepNodes.forEach((node, i) => {
          ...
          gsap.set(node.label, { color: i === stepNodes.length - 1 ? '#0094ac' : '#ffffff' });
          gsap.set(node.desc, { color: 'rgba(255,255,255,0.4)' });
          gsap.set(node.time, { color: 'rgba(255,255,255,0.2)' });
          gsap.set(node.icon, { color: i === stepNodes.length - 1 ? '#0094ac' : 'rgba(255,255,255,0.45)' });
        });
        gsap.set(connectors, { backgroundColor: '#0094ac' });
        gsap.set(summaryRef.current, { display: 'block', opacity: 1, y: 0, scale: 1 });
      };
      ...
      gsap.set(summaryRef.current, { display: 'none', opacity: 0, y: 15, scale: 0.98 });

      stepNodes.forEach((node) => {
        gsap.set(node.core, { backgroundColor: 'rgba(255,255,255,0.2)', scale: 1 });
      ...
      steps.forEach((_, i) => {
        ...
        tl.to(
          node.icon,
          {
            color: i === steps.length - 1 ? '#0094ac' : 'rgba(255,255,255,0.45)',
            duration: 0.3,
          },
          '-=0.7',
        );
  ...
  return (
    <section ref={sectionRef} id="demo" className="bg-[#FAFAFA] py-24 text-brand-blue md:py-32">
      <div className="mx-auto w-full max-w-[1250px] px-6 md:px-12">
        ...
        >
          ...
          <div
            ...
            data-header
          >
            <span className="font-mono text-xs text-white/40">{demoTransaction.id}</span>
            <div className="flex items-center gap-3">
              <span className="text-lg font-semibold tabular-nums text-white">{demoTransaction.amount}</span>
          ...
          <div className="mb-10">
            <div className="h-1.5 w-full overflow-hidden rounded-full bg-white/[0.06]">
              <div ref={progressRef} className="h-full rounded-full bg-brand-teal" style={{ width: '0%' }} data-progress-fill />
            </div>
          </div>

          <div className="mb-8 flex flex-col gap-6 md:flex-row md:gap-4" data-steps>
