  ...
  const renderWords = (text: string, className: string) =>
    text.split(' ').map((word, i) => (
      <span key={`${word}-${i}`} className="inline-block overflow-hidden mr-3">
        <span ref={addRef} className={`inline-block ${className}`} style={{ willChange: 'transform, opacity' }}>
          {word}
        </span>
      </span>
    ));
...
function ProblemIcon({ type }: { type: ProblemItem['icon'] }) {
  const paths: Record<ProblemItem['icon'], string> = {
    ...
    trust: 'M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6-10-6-10-6m10 3a3 3 0 100-6 3 3 0 000 6',
  };
...
function ProblemCard({ item, index, cardRef }: ProblemCardProps) {
  ...
  return (
    <div
      ...
      style={{
        background: 'radial-gradient(ellipse at 30% 20%, rgba(0,148,172,0.08), transparent 70%), #ffffff',
    ...
    >
      ...
      <div className="relative z-10">
        ...
        <div className="h-[2px] bg-brand-teal mb-6 origin-left w-0" data-accent-line />
        <h3 className={`font-semibold text-brand-blue mb-3 ${isWide ? 'text-2xl' : 'text-xl'}`} data-title>
          {item.title}
        </h3>
        <p className="text-sm text-brand-blue/65 leading-relaxed font-light max-w-[32ch]" data-desc>
          {item.desc}
        </p>
      </div>
    </div>
...
export function ProblemSection() {
    ...
    () => {
      ...
      cardsRef.current.forEach((card) => {
        ...
        subTl
          ...
          .to(path, { strokeDashoffset: 0, duration: 0.6, ease: 'power2.out' }, '-=0.5')
          .from(title, { y: 20, opacity: 0, duration: 0.45, ease: 'power2.out' }, '-=0.3')
          .from(desc, { y: 15, opacity: 0, duration: 0.45, ease: 'power2.out' }, '-=0.3')
          .to(index, { opacity: 0.03, duration: 0.35, ease: 'power2.out' }, '<');
      });

      tl.from(quoteRef.current, { y: 30, opacity: 0, duration: 0.6, ease: 'power2.out' }, '-=0.3');
      const quoteLine = quoteRef.current?.querySelector('[data-quote-line]');
      if (quoteLine) {
        tl.to(quoteLine, { height: '100%', duration: 0.6, ease: 'power2.out' }, '-=0.45');
      }
    },
    { scope: sectionRef },
  );
  ...
  return (
    <section ref={sectionRef} id="problem" className="problem-section relative py-24 md:py-32 bg-[#f7fafc] overflow-hidden">
      <Container className="relative z-10">
        <KineticHeading main="Cross-Border" accent="Transfers Are Broken" wordsRef={headingWordsRef} />
        <div className="mx-auto mb-14 h-px bg-brand-teal/80" ref={headingLineRef} />

        <div className="problem-grid">
          {problemItems.map((item, i) => (
            <ProblemCard key={item.title} item={item} index={i} cardRef={setCardRef} />
          ))}
        </div>

        <div ref={quoteRef} className="mt-20 relative pl-8">
          <div className="absolute left-0 top-0 w-[2px] bg-brand-teal origin-top h-0" data-quote-line />
